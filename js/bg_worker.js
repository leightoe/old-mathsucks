!function () {
    "use strict";
    let scripts = ["../include/three.min.js"];

    globalThis.addEventListener("message", (({ data: a }) => {
        if ("undefined" != typeof WebGLRenderingContext)
            if ("init" === a) {
                globalThis.importScripts(...scripts);
                globalThis.postMessage("doneinit");
            } else if (a.canvas && a.windowDPR && a.windowIW && a.windowIH) {
                const canvas = a.canvas,
                    canvasOW = a.canvasOW,
                    canvasOH = a.canvasOH,
                    windowDPR = a.windowDPR,
                    windowIW = a.windowIW,
                    windowIH = a.windowIH;

                globalThis.postMessage("got canvas context");
                ...
                renderer.setPixelRatio(windowDPR > 1 ? 2 : 1);
                ...
                function onMouseMove(e) {
                    mouse.x = (e.clientX / windowIW) - 0.5;
                    mouse.y = (e.clientY / windowIH) - 0.5;
                ...
                globalThis.postMessage("wrong input object type/message" + `master said : ${a}`)
            }
    }
    ), {
        passive: !0
